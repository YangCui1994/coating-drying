...
向上的是 NMP 蒸发/迁移通量。毛细压力 $p_c=p_g-p_l$ 是弯月面两侧的压力差尺度；其局部结果通常是液桥和弯月面使颗粒相互靠近、孔隙缩小。只有当这种致密化包含**面内自由收缩分量**，并且该分量后来受到约束时，才会转化为面内拉应力。如果全部收缩都能通过厚度减薄完成，而面内自然长度不变，则这条路径不会产生面内拉应力。

### 2.2 为什么“想从 100 变成 97”会变成拉伸

定义 $\varepsilon_{\rm free}<0$ 为取出一小块骨架、解除所有面内约束后，它自己会达到的自由收缩应变；$\varepsilon_{\rm actual}$ 是粘在铝箔上以后真正实现的面内应变。忽略不可逆重排时：

$$
\varepsilon_{\rm elastic}
=
...
\sigma_\parallel
=
E'\varepsilon_{\rm elastic}.
$$

若自由膜从 100 缩到 97，则 $\varepsilon_{\rm actual}=\varepsilon_{\rm free}$，所以 $\sigma_\parallel=0$。若骨架的自然长度已经想变成 97，但铝箔使实际长度仍接近 100，则：

$$
\varepsilon_{\rm actual}\approx0,
\qquad
\varepsilon_{\rm free}<0
\quad\Rightarrow\quad
\sigma_\parallel
\approx
-E'\varepsilon_{\rm free}>0.
$$

因此铝箔不需要主动向两边“拉”涂层；它只要让涂层达不到自己的更短自然长度，涂层相对于无应力自然状态就已经被拉长。$E'$ 是与实际单向/双向约束状态一致的湿态有效面内模量，不能直接用最终干膜 Young 模量替代。

### 2.3 真实复合体与锁定/松弛

“实际应变严格为零”是刚性铝箔极限。真实涂层–箔复合体可以略微缩短或弯曲，但界面粘结要求位移协调。在没有外加轴力的简化双层模型中：

$$
N_c+N_f=0.
$$

涂层的自由收缩幅度大于复合体实际缩短幅度，所以涂层承担拉力；铝箔承担平衡压缩力。卷材张力、热膨胀和辊间边界会进一步改变共同实际应变，必须与干燥自由收缩分开记账。

此外，应力必须由已经形成的承载骨架保存。锁定以前，颗粒重排和浆料流动可以释放大部分失配；锁定以后继续失去 NMP，$\varepsilon_{\rm free}$ 进一步变负，而松弛速度若追不上加载速度，$\sigma_\parallel$ 才持续积累。Lewis 等的同步失重–曲率实验直接观察到干燥应力上升、峰值和松弛，并显示配方塑化显著改变应力历史。[直接应力实验：Lewis et al., 1996](https://doi.org/10.1111/j.1151-2916.1996.tb08099.x) 其氧化铝–PVB 参数不能移植给 LFP/PVDF–NMP。
...
1. 存在面内自由收缩分量；
2. 骨架已经锁定并能承载；
3. 存在铝箔、邻层或卷材边界提供的面内约束；
4. 收缩加载不被流动、黏弹或滑移完全松弛。

形成拉应力后，厚度还会通过储能放大裂纹扩展能力。均匀薄层近似为：
...
\qquad
G_{\rm ch}\geq\Gamma_c.
$$

所以厚度增加并不要求应力本身一定成比例增大；仅仅因为可释放弹性能随 $h$ 增加，也可能跨过临界开裂厚度。气泡、团聚体和局部弱层会降低起裂门槛，但不是受约束颗粒膜达到足够厚度后开裂的必要条件。

最后仍要区分：厚向状态不一致首先产生 $\sigma_\parallel(z)$ 和弯曲倾向；只有膜内合力 $N_x=\int\sigma_{xx}dz$ 沿面内位置发生变化时，边缘、裂纹或厚度过渡区才需要明显的界面剪切传递载荷。详细推导见第 6 节。

## 3. A→B：溶剂输运怎样留下厚向状态梯度
