...
> **版本：** 2026-07-30；v2 增加极片剖面过程图，并把原参数流程图移为补充图。  
...
*图 22-1。同一块厚极片的干燥过程示意。A：湿膜中的 LFP、CBD、NMP、气泡和团聚；B：NMP 蒸发后，孔隙弯月面形成的毛细负压促使颗粒骨架收缩，铝箔和已经承载的结构限制面内自由收缩，因此固体骨架中出现面内拉应力；C：厚膜中不同高度可能具有不同的含液状态、锁定时刻、模量、自由收缩量和松弛能力，层间粘结与铝箔约束把这种差异转化为应力差和局部沿面载荷传递；D：表面裂纹、层内裂纹、气泡/空洞、脱层和表面起伏竞争出现。图中蓝色长箭头只表示 NMP 输运/蒸发，紫色孔隙箭头表示毛细负压作用，红色表示固体应力或损伤。图为本项目机制综合，不是任何单一论文原图，也不表示商用 LFP 已经标定的数值窗口。*

### 1.1 对外讨论时，按 A–D 四步说明

1. **先看湿膜里有什么。** 厚度增加不只意味着“材料更多”，还意味着单位面积 NMP 库存和输运距离通常增加。气泡、硬团聚、湿厚热点和界面污染是可能的局部弱点，但它们不是多孔颗粒膜发生开裂的必要条件。
2. **再看收缩为什么变成拉应力。** 蒸发形成的毛细负压主要促使颗粒靠拢和孔隙缩小；它不是一股把膜向上拉裂的力。若湿膜可以完全自由地在面内缩短，拉应力可以很小；实际极片与铝箔粘结，已锁定区域也会约束尚在收缩的区域，于是“想缩但缩不了”的自由收缩被转化为面内拉应力。该因果结构与受约束胶体颗粒膜模型一致。[`Model/D2`：Tirumkudulu & Russel, 2005](https://doi.org/10.1021/la048298k)
3. **厚膜的困难是不同时。** 表层和底层不必严格按一个清晰前沿依次干燥，但厚膜、低渗孔网和较高蒸发通量会提高维持供液所需的压力差，并增加不同高度在含液量、锁定、模量和松弛方面不同步的可能性。只有这些层之间存在收缩或力学差异、又通过层间粘结和基底传递载荷时，才会形成明显的厚向面内应力差或局部界面剪切；“有浓度梯度”本身并不自动等于“有界面剪切”。
4. **最后看哪一种失效先达到条件。** 储存弹性能较大、局部断裂抗力较低或缺陷较大时，可以先出现表面/通道裂纹；气泡或弱层也可能扩展为内部空洞；裂纹触及基底后还可能诱发脱层。Dawson 在 NMC622/PVDF–NMP 小样中直接观察到预存气泡与部分主裂纹位置相关，也观察到裂纹触底后诱发脱层，但这不能外推成商用 LFP 中所有空洞或脱层的统一路径。[`Obs/B`：Dawson et al., 2026](https://doi.org/10.1039/D5EB00201J)

对外讨论时可以先用下面这段话概括：

> 厚极片更容易开裂，并不是因为温度直接把材料烤裂，也不是因为毛细力把极片向上拉开。NMP 蒸发会让颗粒骨架产生收缩倾向；铝箔、层间粘结和已经锁定的结构阻止各处按照自己的节奏自由收缩，于是固体骨架中积累面内拉应力。厚度增加以后，输运路径更长、不同高度的状态更容易不同步，同时可供裂纹释放的能量也增加。气泡和团聚会进一步降低局部起裂门槛，但即使没有气泡，受约束的颗粒膜超过其过程临界厚度后仍可能开裂。

### 1.2 这张图有意避免的三种过度简化

- **没有把毛细作用画成单一向上的力。** 向上的蓝色箭头是液体补给和蒸发方向；毛细负压通过孔壁作用于颗粒骨架，开裂的直接力学驱动力仍是固体骨架中的拉应力和可释放能量。
- **没有把“顶部较干”直接命名为结皮。** 只有测得表层相对内部具有持续较低的渗透率/扩散率或较高的承载能力，才把 `Skin+` 作为已确认状态；表面发哑、粗糙或 PVDF 富集本身不够。[`Obs/Limit/C`：Jaiser et al., 2017](https://doi.org/10.1016/j.jpowsour.2017.01.117)
- **没有把 D1–D5 画成固定先后顺序。** 它们是竞争且可能耦合的失效，表面完整并不能排除层内裂纹或空洞。

若需要从设备、浆料和界面参数继续追到状态变量与 D1–D5，可查看补充的[参数—状态—失效流程图](images/15_fixed_thickness_cracking_control_map.svg)。它适合内部技术分析，不作为对外说明的第一张图。
...
| 优先级 | 可调参数 | 希望改变的中间量 | 候选方向 | 主要反作用与证据边界 |
|---:|---|---|---|---|
...
| 8 | **改变单次干燥层结构**：两次薄涂并分别干燥、顺应性底层或功能梯度 | 单次承载层的有效厚度、裂纹偏折/终止和界面耗散 | 若单次厚涂在可接受产能下仍超过过程 CCT，把总厚度拆成每次低于其 CCT 的干燥层 | 两层湿碰湿若仍共同锁定，不会自动把有效厚度减半；分次干燥增加工序、界面和电阻风险。属于本项目力学推论，尚非目标 LFP 已验证处方。[`Proj`] |
...
| 降低敏感阶段的蒸发峰值、提高固含、优化孔网/PVDF 网络和界面可提高目标 LFP 工艺 CCT | 本项目推论，`Proj` | 方向可能非单调；不能从现有公开文献给出通用温度、线速、固含或配方数值 |
